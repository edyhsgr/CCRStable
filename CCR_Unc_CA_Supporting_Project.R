...
##########
	...
	for (i in 1:ITER) {SxFStart[length(SxFStart[,1]),i]<-sum(LxFStart[HALFSIZE:(HALFSIZE+4),i])/sum(LxFStart[(HALFSIZE-1):(HALFSIZE+4),i])}	

	##(OPEN-ENDED AGE GROUP OPTION (MALE))
	for (i in 1:ITER) {SxMStart[length(SxMStart[,1]),i]<-sum(LxMStart[HALFSIZE:(HALFSIZE+4),i])/sum(LxMStart[(HALFSIZE-1):(HALFSIZE+4),i])}
	...
	##(OPEN-ENDED AGE GROUP OPTION (FEMALE))
	for (i in 1:ITER) {SxFAdj[length(SxFAdj[,1]),i]<-sum(LxFAdj[HALFSIZE:(HALFSIZE+4),i])/sum(LxFAdj[(HALFSIZE-1):(HALFSIZE+4),i])}	

	##(OPEN-ENDED AGE GROUP OPTION (MALE))
	for (i in 1:ITER) {SxMAdj[length(SxMAdj[,1]),i]<-sum(LxMAdj[HALFSIZE:(HALFSIZE+4),i])/sum(LxMAdj[(HALFSIZE-1):(HALFSIZE+4),i])}
